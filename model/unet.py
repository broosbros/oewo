...
class FeatureWiseAffine(nn.Module):
    """
    Feature-wise affine transformation module.
    Args:
        in_channels (int): The number of input channels.
        out_channels (int): The number of output channels.
        use_affine_level (bool): Whether to use affine level.
    Output shape:
    ...
    def __init__(self, in_channels, out_channels, use_affine_level=False):
        ...
        self.noise_func = nn.Linear(in_channels, out_channels * (1 + use_affine_level))

    def forward(self, x, noise_embed):
        batch = x.size(0)
        noise = self.noise_func(noise_embed).view(batch, -1, 1, 1)
...
class ResSE(nn.Module):
    ...
    Args:
        ...
        reduction (int): The reduction ratio.
    Output shape:
        (batch_size, ch_in, h, w)
    """
...
class FD_Info_Spliter(nn.Module):
    """
    Frequency-domain information spliter.
    Args:
    ...
    def forward(self, x, noise_embed):
        ...
        noise_embed = (
            noise_embed.unsqueeze(1).unsqueeze(2).repeat(1, 3, self.image_size, 1)
        )
        ...
        sigma_pre = (
            torch.abs(torch.mean(self.avg_pool(self.sigma_resSE(x_fd)), dim=1))
            + self.image_size / 2
        )
        sigma_min = (
            torch.tensor(self.image_size - 10, device=device)
            .view(1, 1, 1)
            .expand_as(sigma_pre)
        )
        ...
        x_real_filterd, x_imag_filterd = torch.real(tensor_filtered_fft), torch.imag(
            tensor_filtered_fft
        )
...
class Block(nn.Module):
    ...
    Args:
        ...
        groups (int): The number of groups for the group normalization.
...
class ResnetBlock(nn.Module):
    """
    Residual block for ResNet architecture with optional affine transformation.
    Args:
        dim (int): Number of input channels.
        dim_out (int): Number of output channels.
        noise_level_emb_dim (int, optional): Dimension of the noise level embedding. Default is None.
        dropout (float, optional): Dropout rate. Default is 0.
        use_affine_level (bool, optional): Whether to use affine level transformations. Default is False.
...
class SelfAttention(nn.Module):
    ...
    Args:
        ...
        norm_groups (int): The number of groups for the group normalization.
    Output shape:
        (batch_size, dim, h, w)
    """
    ...
    def forward(self, x):
        ...
        normed = self.norm(x)
        qkv = self.qkv(normed).view(batch, self.n_head, head_dim * 3, height, width)
        ...
        out = torch.einsum("bnhwyx, bncyx -> bnchw", attn, value).view(
...
class ResnetBlocWithAttn(nn.Module):
    ...
    Args:
        ...
        norm_groups (int, optional): Number of groups for Group Normalization. Default is 32.
        dropout (float, optional): Dropout rate. Default is 0.
        with_attn (bool, optional): Whether to use self-attention. Default is False.
    Output shape:
        (batch_size, dim_out, h, w)
    """
...
class UNet(nn.Module):
    def __init__(
        ...
        self.fd_spliter = FD_Info_Spliter(
            dim=inner_channel, in_channels=3, out_channels=3, image_size=image_size
        )
...
if __name__ == "__main__":
    img = torch.randn(2, 6, 128, 128)
    t = torch.tensor([[0.645], [0.545]])
    net = UNet()
    y = net(img, t)
    print(y.shape)
